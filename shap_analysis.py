...
class SHAPAnalysisModule(QWidget):
    ...
    def _create_simple_waterfall(self, ax, shap_vals, sample_idx):
        """Create a simple waterfall plot with improved error handling"""
        try:
            print(f"🔍 Creating simple waterfall plot for sample {sample_idx}")
            ...
            n_features = min(15, len(indices_and_vals))
            ...
            for bar, val in zip(bars, values):
                if abs(val) > 0.001:  # Only show significant values
                    width = bar.get_width()
                    label_x = width + (0.01 * max(abs(min(values)), abs(max(values))) if width >= 0 else -0.01 * max(abs(min(values)), abs(max(values))))
                    ax.text(label_x, bar.get_y() + bar.get_height()/2,
                           f'{val:.3f}', ha='left' if width >= 0 else 'right', va='center', fontsize=8)

            print(f"✅ Simple waterfall plot created with {n_features} features")

        except Exception as e:
            print(f"❌ Simple waterfall creation failed: {e}")
    ...
    def _create_simple_force_plot(self, ax, shap_vals, sample_data, sample_idx):
        """Create a simple force plot visualization with improved error handling"""
        try:
            print(f"🔍 Creating simple force plot for sample {sample_idx}")
            ...
            try:
                if hasattr(sample_data, 'iloc'):
                    ...
                    top_feature_vals = [sample_data[i] if i < len(sample_data) else 0 for i in sorted_indices]
            except Exception as data_error:
                print(f"⚠️ Feature value extraction failed: {data_error}")
                top_feature_vals = [0] * len(sorted_indices)

            # Create force plot style visualization
            base_value = getattr(self, 'base_value', 0)

            # Create horizontal bar chart
            y_pos = range(len(top_shap_vals))
            ...
            labels = []
            for i, (name, val) in enumerate(zip(top_feature_names, top_feature_vals)):
                try:
            ...
            # Add base value and prediction info
            prediction = base_value + sum(shap_vals)
            ax.set_xlabel('SHAP Value')
            ax.set_title(f'SHAP Force Plot - Sample {sample_idx}\nBase: {base_value:.3f}, Prediction: {prediction:.3f}')
            ax.axvline(x=0, color='black', linestyle='-', alpha=0.5)
            ax.grid(True, alpha=0.3, axis='x')

            print(f"✅ Simple force plot created with {len(top_shap_vals)} features")

        except Exception as e:
            print(f"❌ Simple force plot creation failed: {e}")
            import traceback
            print(f"❌ Traceback: {traceback.format_exc()}")
