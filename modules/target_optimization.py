...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QGridLayout,
                           ...
                           QFormLayout, QSlider, QStackedWidget, QAbstractItemView)
    ...
    def _apply_categorical_constraints(self, x):
        ...
        for base_name, indices in one_hot_groups.items():
            if len(indices) > 1:  # Only apply if there are multiple features in the group
                # CRITICAL FIX: Check for fixed values first
                fixed_indices = []
                for idx in indices:
                    feature_name = self.feature_names[idx]
                    bounds = self.bounds_dict.get(feature_name)
                    if bounds and bounds.is_fixed:
                        fixed_indices.append(idx)
                        # Force the fixed value
                        x_constrained[idx] = bounds.fixed_value
                
                # If there are fixed values, respect them and adjust others
                if fixed_indices:
                    # Find which fixed index has value 1 (if any)
                    winner_idx = None
                    for idx in fixed_indices:
                        if x_constrained[idx] >= 0.5:  # Fixed to 1
                            winner_idx = idx
                            break
                    
                    # Set all non-fixed to respect the fixed winner
                    for idx in indices:
                        if idx not in fixed_indices:
                            x_constrained[idx] = 0.0  # Set non-fixed to 0
                    
                    # If no fixed winner, set the first fixed to 1
                    if winner_idx is None and fixed_indices:
                        x_constrained[fixed_indices[0]] = 1.0
                        
                else:
                    # No fixed values, apply normal One-Hot constraint
        ...
        for i, feature_name in enumerate(self.feature_names):
            ...
            # CRITICAL FIX: Check for fixed values first - they override everything
            if bounds.is_fixed:
                x_constrained[i] = bounds.fixed_value
                if debug_output:
                    changes_made.append(f"Feature {i} ({feature_name}): {original_value:.4f} -> {x_constrained[i]:.4f} (FIXED)")
                continue  # Skip other constraints for fixed features
...
class TargetOptimizationModule(QWidget):
    ...
    def create_configuration_panel(self):
        ...
        self.constraints_table.setSelectionBehavior(QAbstractItemView.SelectRows)  # Enable row selection
        constraints_layout.addWidget(self.constraints_table)
        
        # Constraint management buttons
        ...
        remove_constraint_btn = QPushButton("Remove Selected")
        remove_constraint_btn.clicked.connect(self.remove_selected_constraint)
        constraint_buttons_layout.addWidget(remove_constraint_btn)
        
        clear_constraints_btn = QPushButton("Clear All")
        clear_constraints_btn.clicked.connect(self.clear_all_constraints)
        constraint_buttons_layout.addWidget(clear_constraints_btn)
        
        constraints_layout.addLayout(constraint_buttons_layout)
    ...
    def remove_selected_constraint(self):
        """Remove the selected constraint from the table"""
        current_row = self.constraints_table.currentRow()
        if current_row >= 0 and current_row < len(self.constraints):
            # Get constraint description for confirmation
            constraint = self.constraints[current_row]
            
            # Ask for confirmation
            reply = QMessageBox.question(self, "Remove Constraint", 
                                       f"Are you sure you want to remove this constraint?\n\n{constraint.description}",
                                       QMessageBox.Yes | QMessageBox.No)
            ...
            if reply == QMessageBox.Yes:
                ...
                QMessageBox.information(self, "Success", "Constraint removed successfully!")
        else:
            QMessageBox.warning(self, "Warning", "Please select a constraint to remove!")
    
    def clear_all_constraints(self):
        """Clear all constraints"""
        if self.constraints:
            reply = QMessageBox.question(self, "Clear All Constraints", 
                                       f"Are you sure you want to remove all {len(self.constraints)} constraints?",
                                       QMessageBox.Yes | QMessageBox.No)
            ...
            QMessageBox.information(self, "Info", "No constraints to clear.")
