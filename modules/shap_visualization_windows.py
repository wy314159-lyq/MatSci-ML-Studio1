...
class SHAPDependenceWindow(QDialog):
    ...
    def create_plot(self):
        ...
        try:
            ...
            # Input validation
            if not self.main_feature or self.main_feature not in self.feature_names:
                self._show_error_message(f"Invalid main feature: {self.main_feature}")
                return
            ...
            # Get feature indices with validation
            try:
                main_idx = self.feature_names.index(self.main_feature)
            except ValueError:
                self._show_error_message(f"Feature '{self.main_feature}' not found in feature list")
                return
            ...
            if self.interaction_feature and self.interaction_feature != "Auto":
                try:
                    ...
                    print(f"Warning: Interaction feature '{self.interaction_feature}' not found, using Auto")
                    interaction_idx = None
    ...
    def _show_error_message(self, message):
        """Display error message on the figure"""
        self.figure.clear()
        ax = self.figure.add_subplot(111)
...
class SHAPLocalWindow(QDialog):
    ...
    def create_plot(self):
        ...
        try:
            ...
            # Input validation
            if self.sample_idx < 0 or self.sample_idx >= len(self.shap_values):
                self._show_error_message(f"Invalid sample index: {self.sample_idx}. Valid range: 0-{len(self.shap_values)-1}")
                return

            if self.shap_values is None or len(self.shap_values) == 0:
                self._show_error_message("No SHAP values available")
                return
            ...
            if self.plot_type == "Waterfall Plot":
                ...
                try:
                    ...
                    print(f"SHAP force_plot failed: {force_error}, using manual fallback")
                    # Manual force plot
                    ax = self.figure.add_subplot(111)
